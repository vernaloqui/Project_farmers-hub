...
function addToCart(addedItem, addedQuantity, addedPrice){
    ...
    for (let i=0; i < cart.length; i++){
      if (cart[i].item == addedItem){
        // cart[i].quantity = cart[i].quantity + addedQuantity;
        // console.log(cart[i].quantity);
        // cart[i].subtotal = cart[i].subtotal + subtotal;
        // cellID = cart[i].item.replace(/ /g, "") +"Cell"; 
        // console.log(cellID);
        // document.getElementById(cellID).value = cart[i].quantity;
        isProductInCart = true;
...
function deleteItemRow(){
    // var itemRow = rowChoice.parentNode.rowIndex;
    // console.log(itemRow);
    // cart = getLSContent();
    // document.getElementById("cartContents").deleteRow(itemRow);
    // // cart.splice(itemRow, 1);
